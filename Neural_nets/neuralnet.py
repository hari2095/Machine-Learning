...
def SGD(train_data,test_data):
    ...
    train_list = []
    test_list  = []
    printProgressBar(0, epochs, prefix = 'Progress:', suffix = 'Complete', bar_length = 100)
    for e in range(epochs):
        J_total = 0.0
        #print "epoch=",e+1
        printProgressBar(e, epochs, prefix = 'Progress:', suffix = 'Complete', bar_length = 100)
        ...
        train_list.append(float(J_total/N))
        ...
        test_list.append(float(J_total/N_test))
        metrics_out.write(test_entr)
        #print train_list
    print ("\n\nplotting the graph.")
    x_axis = [epoch+1 for epoch in range(epochs)]
    plt.plot(x_axis,train_list,'b',label='train entropy')
    plt.plot(x_axis,test_list ,'g',label='test entopy')
    plt.ylabel("Average cross entropy")
    plt.xlabel("Epochs")
    plt.axis([-1,epochs+10,0,1])
